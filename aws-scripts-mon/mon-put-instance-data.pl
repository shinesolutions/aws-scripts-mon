...
if ($report_disk_space)
{
  ...
  foreach my $line (@df)
  {
    ...
    # Switch fsystem to use the same value as mount, this is because the default
    # fsystem relies on `df` command's filesystem output, which value is not
    # guaranteed to be the same as device name. E.g. a /dev/xvda device name
    # can result in /dev/xvda2 fsystem, which makes it hard to filter the
    # metrics at a later stage. We switch the fystem to use the same value as
    # mount here to allow predictability, i.e. mount value is passed via
    # disk-path command arg, it is defined by user, which allows the user to
    # define the same value when filtering the metric at later stage.
    my $fsystem = $fields[5];
    # my $fsystem = $fields[0];
    my $mount = $fields[5];
